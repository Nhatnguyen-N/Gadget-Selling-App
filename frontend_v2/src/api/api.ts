...
export const getMyOrders = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw new Error('An error occurred while fetching orders:' + error.message);
      return data;
    }
  })
}
  ...
  return useMutation({
    async mutationFn({ totalPrice }: { totalPrice: number }) {
      const { data, error } = await supabase
        .from('order')
        .insert({
          totalPrice, slug, user: id, status: 'Pending',
        })
        .select('*')
        .single()
      if (error) throw new Error('An error occurred while creating order:' + error.message);
  ...
  return useMutation({
    async mutationFn(
      ...
      const { data, error } = await supabase
        .from('order_item')
        .insert(
          inserData.map(({ orderId, quantity, productId }) => ({
            order: orderId,
            product: productId,
            quantity
          }))
      ...
      const productQuantities = inserData.reduce((acc, { productId, quantity }) => {
        if (!acc[productId]) {
          acc[productId] = 0;
        }
...
export const getMyOrder = (slug: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error || !data) throw new Error('An occurred while fetching data:' + error.message);
