...
export const createProduct = async ({
  ...
  const { data, error } = await supabase.from('product')
    .insert({ category, heroImage, imagesUrl: images, maxQuantity, price, slug, title })

  if (error) {
    throw new Error(`Èrror creating product: ${error.message}`);
  }
  return data
};
...
export const updateProduct = async ({
  ...
  const { data, error } = await supabase.from('product')
    .update({
      category, heroImage, imagesUrl, maxQuantity, price, title
    }).match({ slug });
  if (error) {
